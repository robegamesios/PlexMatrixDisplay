...
const char * jReadCountObject( const char *pJson, struct jReadElement *pResult, int keyIndex )
{
	...
	}else
	{
		while( 1 )
		{
			...
			pJson= jRead( ++pJson, emptyString.c_str(), &jElement );
			if( pResult->error )
				break;
			pJson= jReadFindTok( pJson, &jTok );
			pResult->elements++;
...
const char * jReadCountArray( const char *pJson, struct jReadElement *pResult )
{
	...
	}else
	{
		while( 1 )
		{
			pJson= jRead( ++pJson, emptyString.c_str(), &jElement );	// array value
			if( pResult->error )
				break;
...
const char *jReadArrayStep( const char *pJsonArray, struct jReadElement *pResult )
{
	...
	case JREAD_COMMA:	// element separator
		return jRead( ++pJsonArray, emptyString.c_str(), pResult );
...
const char * jReadParam( const char *pJson, const char *pQuery, struct jReadElement *pResult, int *queryParams )
{
	...
	case JREAD_OBJECT:		// "{"
		...
		while( 1 )
		{
			...
			pJson= jRead( ++pJson, emptyString.c_str(), pResult );
			pJson= jReadFindTok( pJson, &jTok );
			if( jTok == JREAD_EOBJECT )
			{
